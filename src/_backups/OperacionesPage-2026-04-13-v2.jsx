...
export default function OperacionesPage() {
  ...
  const abrirLead = async (lead) => {
    setForm({
      asignados: lead.asignados || (lead.tecnicoId ? [{ id: lead.tecnicoId, nombre: lead.tecnicoNombre || '' }] : []),
  ...
  const guardarAsignacion = async () => {
    if (form.asignados.length === 0 || !form.fechaEstimada) {
      alert('Asigná al menos una persona y fecha estimada')
      return
    }
    setGuardando(true)
    try {
      ...
      if (modalLead.vendedorId) {
        await crearNotificacion({
          ...
          cuerpo: `"${modalLead.nombre}" asignado a ${nombresAsignados}. Fecha: ${fmtFecha(form.fechaEstimada)}${form.horaEstimada ? ` ${form.horaEstimada}` : ''}. Coordiná con el cliente.`,
            ...
            <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {modalLead.estadoOperacion === 'completado' ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {/* Asignados */}
                  <div>
                    <label style={s.lbl}>Asignado a * ({form.asignados?.length || 0})</label>
                    {form.asignados?.length > 0 && (
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 6 }}>
                        {form.asignados.map(a => (
                          <span key={a.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 12, background: '#E6F1FB', color: '#185FA5', fontSize: 11, fontWeight: 500 }}>
                            {a.nombre}{a.cargo ? ` · ${a.cargo}` : ''}
                            <button onClick={() => setForm({ ...form, asignados: form.asignados.filter(x => x.id !== a.id) })} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#185FA5', fontSize: 13, padding: 0, lineHeight: 1 }}>×</button>
                          </span>
                        ))}
                      </div>
                    )}
                    <div style={{ background: '#f8f9fb', border: '1px solid #eaecf0', borderRadius: 8, maxHeight: 160, overflowY: 'auto' }}>
                      {(() => {
                        const cargosOrdenados = [...new Set(empleados.map(e => e.cargoNombre || 'Sin cargo'))]
                        const cargosUnicos = cargosOrdenados
                        return cargosUnicos.map(cargo => {
                          const emps = empleados.filter(e => (e.cargoNombre || 'Sin cargo') === cargo)
                          if (emps.length === 0) return null
                          return (
                            <div key={cargo}>
                              <div style={{ padding: '5px 10px', fontSize: 9, fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: '.5px', background: '#f0f2f5', borderBottom: '1px solid #e8ecf0' }}>{cargo}</div>
                              {emps.map(emp => {
                                const yaAsignado = form.asignados?.some(a => a.id === emp.id)
                                return (
                                  <div key={emp.id} onClick={() => {
                                    if (yaAsignado) setForm({ ...form, asignados: form.asignados.filter(a => a.id !== emp.id) })
                                    else setForm({ ...form, asignados: [...(form.asignados || []), { id: emp.id, nombre: `${emp.nombre} ${emp.apellido}`, cargo: emp.cargoNombre || '' }] })
                                  }} style={{ padding: '6px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, background: yaAsignado ? '#E6F1FB' : 'transparent', borderBottom: '0.5px solid #f0f2f5' }}
                                  onMouseEnter={e => { if (!yaAsignado) e.currentTarget.style.background = '#fafafa' }}
                                  onMouseLeave={e => { if (!yaAsignado) e.currentTarget.style.background = 'transparent' }}>
                                    <div style={{ width: 16, height: 16, borderRadius: 4, border: yaAsignado ? 'none' : '1.5px solid #ccc', background: yaAsignado ? 'var(--eco-primary, #185FA5)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                      {yaAsignado && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}
                                    </div>
                                    <span style={{ fontWeight: yaAsignado ? 500 : 400 }}>{emp.nombre} {emp.apellido}</span>
                                  </div>
                                )
                              })}
                            </div>
