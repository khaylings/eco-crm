...
const WIDGET_DEFAULTS = {
  // Header / Footer
  logo_principal:  { w:160, h:60,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#185FA5', fontSize:14, fontWeight:'500', text:'', label:'Logo principal' },
  logo_secundario: { w:120, h:50,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#3B6D11', fontSize:12, fontWeight:'400', text:'', label:'Logo secundario' },
  isotipo:         { w:80,  h:80,  bg:'transparent', opacity:0.08, borderRadius:50, borderWidth:0, borderColor:'#d0d8d0', color:'#185FA5', fontSize:12, fontWeight:'400', text:'', label:'Isotipo' },
  texto_libre:     { w:200, h:40,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#1a1a1a', fontSize:14, fontWeight:'400', text:'Texto aquí', label:'Texto libre' },
  nombre_empresa:  { w:220, h:40,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#1a3a5c', fontSize:16, fontWeight:'500', text:'Nombre de la empresa', label:'Nombre empresa' },
  datos_empresa:   { w:220, h:70,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#3a4a5c', fontSize:11, fontWeight:'400', text:'Cédula: X-XXX-XXXXXX\nTel: +506 XXXX-XXXX\nEmail: empresa@mail.com', label:'Datos empresa' },
  numero_proy:     { w:220, h:36,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#185FA5', fontSize:18, fontWeight:'500', text:'PRY-000001', label:'N° proyecto' },
  fechas:          { w:220, h:50,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#555555', fontSize:11, fontWeight:'400', text:'Fecha: 01/01/2026\nResponsable: Nombre', label:'Fechas' },
  datos_cliente:   { w:240, h:80,  bg:'#f8fafc',    opacity:1,    borderRadius:6,  borderWidth:1, borderColor:'#e0e8f0', color:'#333333', fontSize:11, fontWeight:'400', text:'Cliente: Nombre\nEmpresa: S.A.', label:'Datos cliente' },
  imagen:          { w:200, h:120, bg:'transparent', opacity:1,    borderRadius:0,  borderWidth:0, borderColor:'#ffffff', color:'#aaaaaa', fontSize:12, fontWeight:'400', text:'', label:'Imagen' },
  separador:       { w:595, h:2,   bg:'#1a3a5c',    opacity:1,    borderRadius:0,  borderWidth:0, borderColor:'#d0d8d0', color:'#1a3a5c', fontSize:12, fontWeight:'400', text:'', label:'Separador' },
  fondo:           { w:595, h:100, bg:'#f0f4f8',    opacity:1,    borderRadius:0,  borderWidth:0, borderColor:'#d0d8d0', color:'#f0f4f8', fontSize:12, fontWeight:'400', text:'', label:'Bloque fondo' },
  // Contenido (flujo)
  tabla:           { w:547, h:130, bg:'#ffffff',     opacity:1,    borderRadius:4,  borderWidth:1, borderColor:'#e0e0e0', color:'#1a3a5c', fontSize:11, fontWeight:'400', text:'', label:'Tabla entregables', marginTop:0, marginBottom:8, align:'left', alignV:'top' },
  totales:         { w:220, h:90,  bg:'#f0f4f8',    opacity:1,    borderRadius:8,  borderWidth:1, borderColor:'#d0e0f0', color:'#185FA5', fontSize:12, fontWeight:'500', text:'', label:'Totales', marginTop:0, marginBottom:8, align:'right', alignV:'top' },
  observaciones:   { w:307, h:80,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#555555', fontSize:11, fontWeight:'400', text:'Observaciones del proyecto...', label:'Observaciones', marginTop:0, marginBottom:8, align:'left', alignV:'top' },
  terminos:        { w:547, h:70,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#666666', fontSize:10, fontWeight:'400', text:'· Tiempo de ejecución según alcance\n· Pagos según avance del proyecto', label:'Términos', marginTop:0, marginBottom:8, align:'left', alignV:'top' },
  alcance:         { w:547, h:80,  bg:'#f8fafc',    opacity:1,    borderRadius:6,  borderWidth:1, borderColor:'#e0e8f0', color:'#333333', fontSize:11, fontWeight:'400', text:'Alcance del proyecto:\n· Ítem 1\n· Ítem 2', label:'Alcance', marginTop:0, marginBottom:8, align:'left', alignV:'top' },
  fases:           { w:547, h:60,  bg:'transparent', opacity:1,    borderRadius:4,  borderWidth:0, borderColor:'#d0d8d0', color:'#555555', fontSize:11, fontWeight:'400', text:'Fase A → Fase B → Fase C', label:'Fases del proyecto', marginTop:0, marginBottom:8, align:'left', alignV:'top' },
}

const CONTENT_TYPES = ['tabla','totales','observaciones','terminos','alcance','fases']
...
const PANEL_SECTIONS = {
  header: {
    groups: [
      ...
      { label:'Proyecto', items:[{type:'numero_proy',label:'N° proyecto'},{type:'fechas',label:'Fechas'},{type:'datos_cliente',label:'Datos cliente'}] },
      { label:'Diseño', items:[{type:'separador',label:'Separador'},{type:'fondo',label:'Bloque fondo'},{type:'texto_libre',label:'Texto libre'}] },
    ]
  },
  content: {
    groups: [
      { label:'Bloques', items:[{type:'tabla',label:'Tabla entregables'},{type:'observaciones',label:'Observaciones'},{type:'totales',label:'Totales'},{type:'alcance',label:'Alcance'},{type:'fases',label:'Fases'},{type:'terminos',label:'Términos'}] },
    ]
  },
  footer: {
    groups: [
...
function SortableItem({ id, children }) {
  ...
  return (
    <div ref={setNodeRef} style={{ transform:CSS.Transform.toString(transform), transition, opacity:isDragging?0.5:1, cursor:'grab' }} {...attributes} {...listeners}>
      {children}
    </div>
  )
}
...
function WidgetContent({ w, empresa }) {
  const txt = { color:w.color, fontSize:w.fontSize, fontWeight:w.fontWeight, lineHeight:1.45, whiteSpace:'pre-wrap', padding:'4px 8px', width:'100%', boxSizing:'border-box' }
  ...
  if (w.type==='tabla') return (
    <table style={{width:'100%',borderCollapse:'collapse',fontSize:w.fontSize}}>
      <thead>
        <tr style={{background:w.color,color:'#ffffff'}}>
          {['Entregable / Actividad','Cant.','Precio unit.','Total'].map(h=>(
            <th key={h} style={{padding:'5px 8px',textAlign:h==='Entregable / Actividad'?'left':'right',fontSize:w.fontSize,fontWeight:500}}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {[['Actividad de ejemplo','1','$000.00','$000.00'],['Materiales y recursos','1','$000.00','$000.00']].map((row,i)=>(
  ...
  if (w.type==='totales') return (
    <div style={{width:'100%',padding:'8px 12px'}}>
      {[['Sin impuestos','$0,000.00'],['IVA 13%','$000.00']].map(([lbl,val])=>(
        <div key={lbl} style={{display:'flex',justifyContent:'space-between',marginBottom:4,fontSize:w.fontSize-1,color:'#666666'}}>
          <span>{lbl}</span><span>{val}</span>
        </div>
      ))}
      <div style={{display:'flex',justifyContent:'space-between',borderTop:`2px solid ${w.color}`,paddingTop:6,fontSize:w.fontSize,fontWeight:500,color:w.color}}>
        <span>Total proyecto</span><span>$0,000.00</span>
      </div>
    </div>
  )
...
function PropsPanel({ widget, onChange, onDelete, onUploadImage, onSubirCapa, onBajarCapa, seccion }) {
  if (!widget) return (
    ...
    <div style={{padding:'10px 12px',overflowY:'auto',flex:1}}>
      ...
      {!isDesign && !isLogo && w.type!=='imagen' && (
        <div>
          ...
          <div style={row}>
            <input type="color" value={safeColor(w.bg)} onChange={e=>onChange('bg',e.target.value)}
              style={{width:28,height:26,padding:1,cursor:'pointer',borderRadius:4,border:'0.5px solid #d0d8d0'}} />
            <button onClick={()=>onChange('bg','transparent')}
              style={{flex:1,padding:'4px',border:'0.5px solid #d0d8d0',borderRadius:5,background:'transparent',cursor:'pointer',fontSize:10,color:'#666666',fontFamily:'inherit'}}>
              Transparente
            </button>
          </div>
        </div>
      ...
      {w.type==='separador' && (
        <div>
          <span style={lbl}>Color línea</span>
          <div style={row}>
            <input type="color" value={safeColor(w.color)} onChange={e=>{onChange('color',e.target.value);onChange('bg',e.target.value)}}
              style={{width:28,height:26,padding:1,cursor:'pointer',borderRadius:4,border:'0.5px solid #d0d8d0'}} />
      ...
      {w.type==='fondo' && (
        <div>
          <span style={lbl}>Color del bloque</span>
          <div style={row}>
            <input type="color" value={safeColor(w.bg)} onChange={e=>onChange('bg',e.target.value)}
              style={{width:28,height:26,padding:1,cursor:'pointer',borderRadius:4,border:'0.5px solid #d0d8d0'}} />
      ...
      {w.type!=='separador' && (
        <div>
          <span style={lbl}>Borde</span>
          <div style={row}>
            <input type="range" min={0} max={8} value={w.borderWidth||0} onChange={e=>onChange('borderWidth',Number(e.target.value))} style={{flex:1}} />
            <input type="color" value={safeColor(w.borderColor)} onChange={e=>onChange('borderColor',e.target.value)}
              style={{width:28,height:26,padding:1,cursor:'pointer',borderRadius:4,border:'0.5px solid #d0d8d0'}} />
          </div>
        </div>
      )}
      ...
      {w.type==='imagen' && (
        <div>
          ...
          <button onClick={()=>onUploadImage(w.id)} style={{width:'100%',padding:'6px',border:'0.5px dashed #d0d8d0',borderRadius:5,background:'transparent',cursor:'pointer',fontSize:11,color:'#888888',fontFamily:'inherit'}}>
            {w.imageUrl?'Cambiar imagen':'+ Subir imagen'}
          </button>
          {w.imageUrl&&(<>
            <span style={lbl}>Modo de ajuste</span>
            <div style={{display:'flex',gap:4,marginBottom:4}}>
              {[['cover','Recortar'],['contain','Contener'],['fill','Estirar']].map(([val,l2])=>(
                <button key={val} onClick={()=>onChange('objectFit',val)}
                  style={{flex:1,padding:'4px 2px',border:`0.5px solid ${(w.objectFit||'cover')===val?'#185FA5':'#d0d8d0'}`,borderRadius:5,background:(w.objectFit||'cover')===val?'#E6F1FB':'transparent',color:(w.objectFit||'cover')===val?'#0C447C':'#666666',fontSize:10,cursor:'pointer',fontFamily:'inherit'}}>
                  {l2}
                </button>
              ))}
            </div>
      ...
      {['texto_libre','nombre_empresa','datos_empresa','numero_proy','fechas','datos_cliente','observaciones','terminos','alcance','fases'].includes(w.type) && (
        <div>
      ...
      {!isDesign && !isLogo && w.type!=='imagen' && (
        <div>
          ...
          <div style={row}>
            <input type="color" value={safeColor(w.color)} onChange={e=>onChange('color',e.target.value)}
              style={{width:28,height:26,padding:1,cursor:'pointer',borderRadius:4,border:'0.5px solid #d0d8d0'}} />
          ...
          <div style={row}>
            {[['400','Normal'],['500','Negrita']].map(([val,l2])=>(
              <button key={val} onClick={()=>onChange('fontWeight',val)}
                style={{flex:1,padding:'4px',border:`0.5px solid ${w.fontWeight===val?'#185FA5':'#d0d8d0'}`,borderRadius:5,background:w.fontWeight===val?'#E6F1FB':'transparent',color:w.fontWeight===val?'#0C447C':'#666666',fontSize:11,cursor:'pointer',fontFamily:'inherit',fontWeight:val}}>
                {l2}
              </button>
            ))}
          </div>
      ...
      {isContent && (
        <div style={{marginTop:14}}>
          ...
          <div style={row}>
            <input type="range" min={0} max={60} value={w.marginTop??0} onChange={e=>onChange('marginTop',Number(e.target.value))} style={{flex:1}} />
            <span style={{fontSize:10,color:'#888888',minWidth:28}}>{w.marginTop??0}px</span>
          </div>
          <span style={lbl}>Margen inferior</span>
          <div style={row}>
            <input type="range" min={0} max={60} value={w.marginBottom??8} onChange={e=>onChange('marginBottom',Number(e.target.value))} style={{flex:1}} />
            <span style={{fontSize:10,color:'#888888',minWidth:28}}>{w.marginBottom??8}px</span>
          </div>
          <span style={lbl}>Alineación horizontal</span>
          <div style={{display:'flex',gap:4,marginBottom:4}}>
            {[['left','◀'],['center','▪'],['right','▶']].map(([val,l2])=>(
              <button key={val} onClick={()=>onChange('align',val)}
                style={{flex:1,padding:'4px 2px',border:`0.5px solid ${(w.align||'left')===val?'#185FA5':'#d0d8d0'}`,borderRadius:5,background:(w.align||'left')===val?'#E6F1FB':'#ffffff',color:(w.align||'left')===val?'#0C447C':'#666666',fontSize:10,cursor:'pointer',fontFamily:'inherit'}}>
                {l2}
              </button>
            ))}
          </div>
          ...
          <div style={{display:'flex',gap:4}}>
            {[['top','▲ Arr'],['middle','▪ Med'],['bottom','▼ Abj']].map(([val,l2])=>(
              <button key={val} onClick={()=>onChange('alignV',val)}
                style={{flex:1,padding:'4px 2px',border:`0.5px solid ${(w.alignV||'top')===val?'#185FA5':'#d0d8d0'}`,borderRadius:5,background:(w.alignV||'top')===val?'#E6F1FB':'#ffffff',color:(w.alignV||'top')===val?'#0C447C':'#666666',fontSize:10,cursor:'pointer',fontFamily:'inherit'}}>
                {l2}
              </button>
            ))}
          </div>
...
export default function PlantillaProyecto() {
  ...
  const getDefaultContent = () => {
    ...
    return [W(150,'tabla'),W(151,'observaciones'),W(152,'totales'),W(153,'terminos')]
  }
  ...
  const cargar = async () => {
    ...
    try {
      const [empSnap, pltSnap] = await Promise.all([
        ...
        getDoc(doc(db,'configuracion','plantilla_proyecto')),
      ])
      ...
      if (pltSnap.exists()) {
        ...
        setTodasPlantillas([...predefs,...personalizadas])

        let cfg = plantillaId.startsWith('custom_')
          ? (personalizadas.find(p=>p.id===plantillaId))
          : d.plantillasConfig?.[plantillaId]
        ...
        if (cfg) {
          if (cfg.headerWidgets) {
            ...
            idCounter.current = Math.max(200,...[...(cfg.headerWidgets||[]),...(cfg.contentWidgets||[]),...(cfg.footerWidgets||[])].map(w=>w.id||0))+1
          } else if (cfg.widgets) {
            const ORDER = ['tabla','observaciones','totales','alcance','fases','terminos']
            ...
            idCounter.current = Math.max(200,...cfg.widgets.map(w=>w.id||0))+1
          }
        } else {
          setHeaderWidgets([])
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (plantillaId.startsWith('custom_')) {
        const snap = await getDoc(doc(db,'configuracion','plantilla_proyecto'))
        ...
        await setDoc(doc(db,'configuracion','plantilla_proyecto'),{plantillasPersonalizadas:nuevas},{merge:true})
      } else {
        await setDoc(doc(db,'configuracion','plantilla_proyecto'),{plantillasConfig:{[plantillaId]:cfg}},{merge:true})
      }
      setMsg('✓ Guardado')
      setHayCambios(false)
  ...
  const handleImageFile = async e => {
    ...
    try {
      const r=ref(storage,`plantillas/proyecto/imagenes/${imgTargetId.current}_${Date.now()}`)
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',fontFamily:'inherit',background:'#f4f6f4'}}>
      ...
      <div style={{background:'#ffffff',borderBottom:'0.5px solid #d0d8d0',padding:'7px 14px',display:'flex',alignItems:'center',gap:8,flexShrink:0,flexWrap:'wrap'}}>
        <span style={{fontSize:12,fontWeight:500,color:'#1a1a1a'}}>Plantilla:</span>
        <select value={plantillaId}
          onChange={e=>{if(hayCambios&&!window.confirm('¿Cambiar sin guardar?'))return;navigate('/configuracion/plantilla-proyecto?id='+e.target.value)}}
          ...
          {todasPlantillas.length>0?todasPlantillas.map(p=><option key={p.id} value={p.id}>{p.nombre}</option>):<option value={plantillaId}>{plantillaId}</option>}
        </select>
          ...
          <div style={{transformOrigin:'top center',transform:`scale(${zoom})`,marginBottom:`${(842*(zoom-1))+24}px`}}>
            <div style={{width:595,minHeight:842,background:'#ffffff',boxShadow:'0 2px 20px rgba(0,0,0,.15)',fontFamily:'Inter,sans-serif',display:'flex',flexDirection:'column'}}>
              ...
              <div ref={seccion==='header'?hojaRef:null}
                style={{position:'relative',width:595,height:headerH,background:'#ffffff',outline:seccion==='header'?'2px solid #185FA5':'none',outlineOffset:-2,overflow:'hidden'}}
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                  <SortableContext items={contentWidgets.map(w=>w.id)} strategy={verticalListSortingStrategy}>
                    {contentWidgets.map((w,i)=>{
                      ...
                      const ai = (w.alignV||'top')==='middle'?'center':(w.alignV||'top')==='bottom'?'flex-end':'flex-start'
                      const wrapStyle = { marginTop:w.marginTop??0, marginBottom:w.marginBottom??8, display:'flex', justifyContent:jc, alignItems:ai, minHeight:w.h||'auto' }

                      if (w.type==='observaciones') {
                        return (
                          <SortableItem key={w.id} id={w.id}>
                            <div style={{...wrapStyle,outline:selectedId===w.id||selectedId===totW?.id?'1px dashed #185FA5':'1px dashed transparent',outlineOffset:2,borderRadius:4}}>
