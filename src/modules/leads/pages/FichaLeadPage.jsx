...
import { collection, addDoc, serverTimestamp, getDocs, doc, getDoc, query, where, orderBy, onSnapshot } from 'firebase/firestore'
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:'#f5f6fa', overflow:'hidden' }}>
      ...
      <div style={{ background:'#fff', borderBottom:'1px solid #e8ecf0', padding:'0 14px', display:'flex', alignItems:'stretch', gap:0, flexShrink:0, height:38 }}>
        ...
        <button onClick={() => setModalAccion('cotizacion')} style={{ ...S.odooBtn, color:'#185FA5', fontWeight:500 }}>+ Nueva cotización</button>
                  ...
                  {obsNotas.length === 0
                    ...
                    : obsNotas.map(n => (
                        <div key={n.id} style={{ background:'#f8f9fc', borderRadius:7, padding:'8px 10px', marginBottom:6, fontSize:12 }}>
                          <div style={{ fontSize:9, color:'#bbb', marginBottom:3 }}>{n.autor} · {n.creadoEn?.toDate?.()?.toLocaleString('es-CR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})||''}</div>
                          {n.texto}
                        </div>
                      ))
                  }
              ...
              {timeline.map(item => (
                <div key={item.id} style={{ display:'flex', gap:8, marginBottom:10, alignItems:'flex-start' }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:9, color:'#bbb', marginTop:1 }}>{item.usuario} · {item.creadoEn?.toDate?.()?.toLocaleString('es-CR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})||''}</div>
                  </div>
                </div>
              ))}
            </div>
                  ...
                  return (
                    <div key={nota.id}>
                      <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
                        ...
                        <span style={{ fontSize:9, color:'#bbb' }}>{nota.creadoEn?.toDate?.()?.toLocaleString('es-CR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})||''}</span>
                      </div>
