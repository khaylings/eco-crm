...
function PanelNotificaciones({ notifs, onMarcarLeida, onEliminar, onMarcarTodasLeidas, onNavegar, onCerrar, onAbrirEliminacion }) {
  ...
  const agrupadas = (() => {
    const facturaNotifs = notifs.filter(n => n.tipo === 'pago_pendiente' && !n.leida)
    const otras = notifs.filter(n => !(n.tipo === 'pago_pendiente' && !n.leida))
    if (facturaNotifs.length <= 1) return notifs
    const grupo = {
      id: 'grupo_facturas',
      _ids: facturaNotifs.map(n => n.id),
      tipo: 'pago_pendiente',
      titulo: `${facturaNotifs.length} facturas pendientes de cobro`,
      cuerpo: facturaNotifs.slice(0, 3).map(n => n.titulo || n.cuerpo).join(' · ') + (facturaNotifs.length > 3 ? ` y ${facturaNotifs.length - 3} más` : ''),
      link: '/finanzas?tab=cxc',
      leida: false,
      creadoEn: facturaNotifs[0]?.creadoEn,
      _esGrupo: true,
    }
    return [grupo, ...otras]
  })()
            ...
            <div key={n.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 16px', borderBottom: '0.5px solid rgba(0,0,0,.05)', background: n.leida ? '#fff' : '#fafbff' }}>
              <div onClick={() => {
                ...
                <div style={{ width: 36, height: 36, borderRadius: 10, background: cfg.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>
                  {n._esGrupo ? <span style={{ fontSize: 12, fontWeight: 700 }}>{n._ids.length}</span> : cfg.icono}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {n.cuerpo && <p style={{ fontSize: 11, color: '#888', margin: '3px 0 0', lineHeight: 1.4, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{n.cuerpo}</p>}
              ...
              <button onClick={e => { e.stopPropagation(); if (n._esGrupo) { n._ids.forEach(id => onEliminar(id)); } else { onEliminar(n.id); } }}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ccc', fontSize: 14, padding: '2px 4px', flexShrink: 0, marginTop: 2, lineHeight: 1 }}
...
export default function MainLayout() {
  ...
  const eliminarNotif = useCallback(async (notifId) => {
  ...
  const aprobarEliminacion = async () => {
    ...
    try {
      ...
      if (meta.coleccion && meta.documentoId) {
        // Si es factura, buscar cotización vinculada y revertir estado
        if (meta.coleccion === 'facturas') {
          try {
