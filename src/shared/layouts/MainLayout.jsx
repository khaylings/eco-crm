...
const MODULOS_GRID_BASE = [
  ...
  { path: '/contactos',       label: 'Contactos',     icono: '👤', color: '#185FA5', bg: '#E6F1FB' },
  { path: '/empresas',        label: 'Empresas',      icono: '🏢', color: '#854F0B', bg: '#FAEEDA' },
...
function GrupoNav({ item, isActive, onNavigate }) {
  ...
  const activo = item.items.some(i => isActive(i.path))
      ...
      {hover && (
        <div onMouseEnter={mostrar} onMouseLeave={ocultar} style={{ position: 'fixed', left: dropPos.left, top: dropPos.top, zIndex: 9999, background: '#fff', borderRadius: '0 0 10px 10px', minWidth: 190, boxShadow: '0 8px 24px rgba(0,0,0,.15)', border: '0.5px solid rgba(0,0,0,.08)', overflow: 'hidden' }}>
          {item.items.map(sub => {
            ...
            return (
              <button key={sub.path} onClick={() => { onNavigate(sub.path); setHover(false) }}
...
export default function MainLayout() {
  ...
  // ── Cambio 3: Finanzas se inserta antes de Configuración (sin tocar Email) ──
  const navItems = esAdmin
    ? [...NAV_ITEMS_BASE.slice(0, -1), { path: '/finanzas', label: '💰 Finanzas' }, NAV_ITEMS_BASE[NAV_ITEMS_BASE.length - 1]]
    : NAV_ITEMS_BASE
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          ...
          <div ref={menuRef} style={{ position: 'relative' }}>
            <button onClick={() => { setMenuOpen(o => !o); setNotifOpen(false) }}
              style={{ width: 30, height: 30, borderRadius: '50%', background: 'rgba(255,255,255,.2)', border: '1.5px solid rgba(255,255,255,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 500, color: '#fff', cursor: 'pointer', overflow: 'hidden', padding: 0 }}>
